...
{
    public partial class frmReporteCotizacionEdgarSierra : Form
    {
        ...
        public frmReporteCotizacionEdgarSierra()
        {
            InitializeComponent();
        }

        private void frmReporteCotizacionEdgarSierra_Load(object sender, EventArgs e)
        {
            this.clientesTableAdapter.FillBy(this.dSCOCOA.Clientes);
            clienteComboBox.SelectedValue = -1;
            if (idCotizacion != 0)
            {
                rptReporteCotizacionEdgarSierra miListado = new rptReporteCotizacionEdgarSierra();
